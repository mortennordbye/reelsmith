...
is what lets the four-call publish sequence be asserted at all. The thing worth
testing here is the sequence and the failure handling, not Meta's JSON.
"""
...
def _happy_handler(calls: list[httpx.Request], *, statuses=("FINISHED",)):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/media"):
            return _json({"id": "container-1", "uri": "https://rupload.example/upload/container-1"})
        if "rupload" in request.url.host or "/upload/" in path:
...
def test_publish_runs_the_four_calls_in_order(cfg, tmp_path):
    calls: list[httpx.Request] = []
    result = publisher.publish_reel(
        _video(tmp_path), "a caption", cfg, client=_client(_happy_handler(calls))
    )
    ...
    assert steps[:2] == ["POST media", "POST container-1"]
    assert "POST media_publish" in steps


def test_the_container_asks_for_a_resumable_reel(cfg, tmp_path):
    calls: list[httpx.Request] = []
    publisher.publish_reel(_video(tmp_path), "hello", cfg, client=_client(_happy_handler(calls)))
...
def test_without_a_cover_url_it_points_at_the_cover_frame(cfg, tmp_path):
    ...
    calls: list[httpx.Request] = []
    publisher.publish_reel(_video(tmp_path), "hi", cfg, client=_client(_happy_handler(calls)))
...
def test_the_upload_uses_the_uri_meta_handed_back(cfg, tmp_path):
    calls: list[httpx.Request] = []
    publisher.publish_reel(_video(tmp_path), "hi", cfg, client=_client(_happy_handler(calls)))

    upload = calls[1]
    assert str(upload.url) == "https://rupload.example/upload/container-1"
    # OAuth, not Bearer: the upload host rejects the Graph scheme.
    assert upload.headers["authorization"] == "OAuth seed-token"
    assert upload.headers["file_size"] == "2048"
...
def test_publish_waits_out_in_progress(cfg, tmp_path):
    ...
    assert publisher.publish_reel(_video(tmp_path), "hi", cfg, client=_client(handler))
...
def test_a_missing_video_fails_before_any_request(cfg, tmp_path):
    ...
    with pytest.raises(PublishError, match="No video"):
        publisher.publish_reel(tmp_path / "gone.mp4", "hi", cfg, client=_client(handler))


def test_the_upload_resumes_from_the_reported_offset(cfg, tmp_path):
    seen: list[str] = []
    attempts = {"n": 0}
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        path = request.url.path
        if path.endswith("/media"):
            return _json({"id": "c", "uri": "https://rupload.example/upload/c"})
        if "/upload/" in path:
            if request.method == "GET":
                return _json({"bytes_transferred": 1024})
            attempts["n"] += 1
            seen.append(request.headers["offset"])
            if attempts["n"] == 1:
                return _json({"error": {"message": "network hiccup"}}, status=500)
            return _json({"success": True})
        if path.endswith("/c"):
