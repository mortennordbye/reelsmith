...
def stage(
    ...
    *,
    ...
    Named `<slug>-art<N>.jpg`, which `renderer.STAGED_ASSET_RE` has to
    recognise or the prune never deletes them. Commons scans are the largest
    files this niche stages, so that mattered enough to write down twice.
    ...
    for i, artefact in enumerate(usable):
        ...
        name = f"{subject.slug}-art{i}{suffix}"
        try:
